...
async fn main() -> Result<()> {
    ...
    let args = Args::parse();
    let Args {
        seed, birth_height, ntaddrs, nzaddrs, destination, lwd_url
    } = args;
    ...
    zec.set_single_lwd_url(lwd_url.to_string())?;
    ...
    let sweep = |table: &'static str| {
        ...
        async move {
            ...
            let zaccounts = rows.collect::<Result<Vec<_>, _>>()?;
